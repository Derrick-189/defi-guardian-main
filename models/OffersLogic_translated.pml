...
active proctype Contract() {
    ...
    do
        :: state == 1 -> atomic {
            ...
            assert(user_debt >= 0);
            assert(price_eth > 0);
            assert(Permissions(address(this); /* Business logic invariant */
            assert(_offersStorage(); /* Business logic invariant */
            assert(_offersStorage(); /* Business logic invariant */
            assert(_targetOffer.expirationTimestamp > block.timestamp); /* Business logic invariant */
            assert(
            _validateERC20BalAndAllowance(_targetOffer.offeror); /* Business logic invariant */
            assert(_startId <= _endId && _endId < _offersStorage(); /* Business logic invariant */
            assert(_startId <= _endId && _endId < _offersStorage(); /* Business logic invariant */
            assert(_params.totalPrice > 0); /* Business logic invariant */
            assert(_params.quantity > 0); /* Business logic invariant */
            assert(_params.quantity == 1 || _tokenType == TokenType.ERC1155); /* Business logic invariant */
            assert(
            _params.expirationTimestamp + 60 minutes > block.timestamp); /* Business logic invariant */
            assert(
            _validateERC20BalAndAllowance(_msgSender(); /* Business logic invariant */
            assert(isValid); /* Business logic invariant */
            assert(amountRemaining >= royaltyCut); /* Business logic invariant */
